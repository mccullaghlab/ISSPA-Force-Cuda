...
dat = np.loadtxt("LJ2.qp0.50.qm1.00.IS-SPA.dn0.21.dat")
...
for i in range(nBins):
    ...
    f[i,1] += tf[i,3]
    f[i,2] += tf[i,6]
plt.plot(f[:,0], -f[:,1], c = 'k', label = "+0.5 ion - cuda")
plt.plot(f[:,0], f[:,2], c = 'g', label = "-1.0 ion - cuda")
...
plt.xlim((0,25))
plt.ylim((0.0,5.0))
plt.savefig('p0.5_m1.0.cforces.pdf')
plt.savefig('p0.5_m1.0.cforces.png')
plt.close()
