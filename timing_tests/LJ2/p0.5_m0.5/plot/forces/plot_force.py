...
q2 = -0.5
p1 =(0,0,0)
dat = np.loadtxt("/home/ryan/Desktop/isspa/isspa2_v3_forces/timing_tests/LJ2/p0.5_m0.5/12/force/rex/LJ2.qp0.50.qm0.50.IS-SPA.dn0.21.dat")
...
#plt.plot(data[:,0], data[:,1], c = 'b', label = "+0.5 ion - IS-SPA")
...
for j,val in enumerate(vals):
    tf = np.loadtxt("/home/ryan/Desktop/isspa/isspa2_v3_forces/timing_tests/LJ2/p0.5_m0.5/%s/LJ2.c_forces.combined.dat" %(val))
...
# Plot the dimer forces
for j,val in enumerate(vals):
    tf = np.loadtxt("/home/ryan/Desktop/isspa/isspa2_v3_forces/timing_tests/LJ2/p0.5_m0.5/%s/LJ2.forces.combined.dat" %(val))
    plt.errorbar(tf[:,0], tf[:,3], yerr=tf[:,4], c = colors[j], label="$r_{cut}$ = %s" %(val), linewidth = 1.0)
...
plt.xlim((10,25))
plt.ylim((-0.4,0.0))
plt.savefig('p0.5_m0.5.tforces.pdf')
plt.savefig('p0.5_m0.5.tforces.png')
plt.close()
