...
dat = np.loadtxt("LJ2.qp0.00.qm0.00.IS-SPA.dn0.21.dat")
data = np.zeros((len(dat),3),dtype=float)
for i in range(len(dat)):
    data[i,0] = dat[i,0]
    data[i,1] =  dat[i,3]+q1*fC(dat[i,0])
    data[i,2] =  dat[i,4]+q2*fC(dat[i,0])
    
# Plot the forces from rex
plt.plot(data[:,0], data[:,1], c = 'b', label = "0.0 - IS-SPA")
plt.plot(data[:,0], data[:,2], c = 'r', label = "0.0 - IS-SPA")
...
for i in range(nBins):
    ...
    f[i,1] += tf[i,3]
    f[i,2] += tf[i,6]
plt.plot(f[:,0],-f[:,1], c = 'k', label = "0.0 - cuda")
plt.plot(f[:,0], f[:,2], c = 'g', label = "0.0 - cuda")
...
plt.xlim((0,25))
plt.ylim((-0.25,0.25))
plt.savefig('n0.0_n0.0.cforces.pdf')
plt.savefig('n0.0_n0.0.cforces.png')
plt.close()
