...
C solvent values
        ...
        double precision e0,p0,q0,o0,rmax,eps
C random
        integer ma(55),inext,inextp
C
        double precision x(3,nmax),f(3,nmax,6)
...
C
        common /solvent/ gr,Emf,fLJ,vtot,e0,p0,q0,o0,rmax,eps,
...
C  Get the positions, velocities, and atom types
        call getx(x,lbox,hlbox,natom,frst)
        call wrap(x,lbox,nmol,nmpnt,ncpu)
C  Get new forces
        call getfrc(x,lbox,hlbox,f)
        open(20,FILE="LJ2.multi.frc.Csolu.dat",STATUS="unknown")
        do i=1,natom
          write(20,*) x(1,i),x(2,i),x(3,i),f(1,i,1),f(2,i,1),f(3,i,1)
        enddo
        close(20)
        open(20,FILE="LJ2.multi.frc.LJsolu.dat",STATUS="unknown")
        do i=1,natom
          write(20,*) x(1,i),x(2,i),x(3,i),f(1,i,2),f(2,i,2),f(3,i,2)
        enddo
        close(20)
        open(20,FILE="LJ2.multi.frc.Csolv_pair.dat",STATUS="unknown")
        do i=1,natom
          write(20,*) x(1,i),x(2,i),x(3,i),f(1,i,3),f(2,i,3),f(3,i,3)
        enddo
        close(20)
        open(20,FILE="LJ2.multi.frc.Csolv.dat",STATUS="unknown")
        do i=1,natom
          write(20,*) x(1,i),x(2,i),x(3,i),f(1,i,4),f(2,i,4),f(3,i,4)
        enddo
        close(20)
        open(20,FILE="LJ2.multi.frc.Csolv_CDD.dat",STATUS="unknown")
        do i=1,natom
          write(20,*) x(1,i),x(2,i),x(3,i),f(1,i,5),f(2,i,5),f(3,i,5)
        enddo
        close(20)
        open(20,FILE="LJ2.multi.frc.LJsolv.dat",STATUS="unknown")
        do i=1,natom
          write(20,*) x(1,i),x(2,i),x(3,i),f(1,i,6),f(2,i,6),f(3,i,6)
        enddo
        close(20)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine getx(x,lbox,hlbox,natom,frst)
        integer nmax
        parameter(nmax=2000)
...
C
        ...
        double precision f(3,nmax,6),dis(3,nmax)
        double precision fij(3,3),fi(3,20,nmax,6)
        ...
        double precision c1,c2,c3,cothe,dp1,dp2,dp3,Rz
C
...
C$OMP&          ,c1,c2,c3,cothe,dp1,dp2,dp3,Rz)
...
C$OMP&          ,mass,frst2,frst1,e0,p0,q0,o0,eps
C$OMP&          ,rmax,rcom)
          ...
          do j=1,natom
            ...
            if (j.gt.i) then
              if (excl(iex).ne.j) then
                ...
                r0=dsqrt(r2)
                if (i.eq.1) then
                   write(6,*) i,j,x(1,i),x(1,j),r0,fdir2 
                endif
...
C
            do j=1,natom
              ...
              r0=dsqrt(r2)
C  Coulomb
              Rz=(enow(1)*dx+enow(2)*dy+enow(3)*dz)/r0
              dp1=3.d0*Rz
              dp2=7.5d0*Rz**2-1.5d0
              dp3=(17.5d0*Rz**2-7.5d0)*Rz
              fs=-charge(j)*p0*c1/r2/r0
              fij(1,1)=fs*(dp1*dx/r0-enow(1))
              fij(2,1)=fs*(dp1*dy/r0-enow(2))
              fij(3,1)=fs*(dp1*dz/r0-enow(3))
              fs=-charge(j)*q0*(1.5d0*c2-0.5d0)/r2**2
              fij(1,1)=fij(1,1)+fs*(dp2*dx/r0-dp1*enow(1))
              fij(2,1)=fij(2,1)+fs*(dp2*dy/r0-dp1*enow(2))
              fij(3,1)=fij(3,1)+fs*(dp2*dz/r0-dp1*enow(3))
              fs=-charge(j)*o0*(2.5d0*c3-1.5d0*c1)/r2**2/r0
              fij(1,1)=fij(1,1)+fs*(dp3*dx/r0-dp2*enow(1))
              fij(2,1)=fij(2,1)+fs*(dp3*dy/r0-dp2*enow(2))
              fij(3,1)=fij(3,1)+fs*(dp3*dz/r0-dp2*enow(3))
C  LJ
              if(r0.lt.rmax) then
                dr=r0/0.1d0+0.5d0
                ir=int(dr)
...
C
              ...
              fij(1,3)=fij(1,3)*gnow
              fij(2,3)=fij(2,3)*gnow
              fij(3,3)=fij(3,3)*gnow
C  Constant dielectric
              if(iin(j).eq.0) then
                fs=-charge(j)*p0/r2/r0
                pdotr=3.d0*(e0now(1)*dx+e0now(2)*dy+e0now(3)*dz)/r2
                fij(1,2)=fs*(pdotr*dx-e0now(1))
                fij(2,2)=fs*(pdotr*dy-e0now(2))
                fij(3,2)=fs*(pdotr*dz-e0now(3))
              else
                fij(1,2)=0.d0
                fij(2,2)=0.d0
                fij(3,2)=0.d0
...
C  Add to forces
              fi(1,tid,j,4)=fi(1,tid,j,4)+fij(1,1)*vtot(it)/dble(igo)
              fi(2,tid,j,4)=fi(2,tid,j,4)+fij(2,1)*vtot(it)/dble(igo)
              fi(3,tid,j,4)=fi(3,tid,j,4)+fij(3,1)*vtot(it)/dble(igo)
C
              fi(1,tid,j,5)=fi(1,tid,j,5)+fij(1,2)*vtot(it)/dble(igo)
              fi(2,tid,j,5)=fi(2,tid,j,5)+fij(2,2)*vtot(it)/dble(igo)
              fi(3,tid,j,5)=fi(3,tid,j,5)+fij(3,2)*vtot(it)/dble(igo)
C
              fi(1,tid,j,6)=fi(1,tid,j,6)+fij(1,3)*vtot(it)/dble(igo)
              fi(2,tid,j,6)=fi(2,tid,j,6)+fij(2,3)*vtot(it)/dble(igo)
              fi(3,tid,j,6)=fi(3,tid,j,6)+fij(3,3)*vtot(it)/dble(igo)
            enddo
          enddo
        enddo
C$OMP end do
...
C            f14v=r6*(alj(nlj)*r6-blj(nlj))/scnb(i5)/r2
C            do k=1,3
C              fi(k,tid,i1,1)=fi(k,tid,i1,1)+f14e*r(k)
C              fi(k,tid,i4,1)=fi(k,tid,i4,1)-f14e*r(k)
C              fi(k,tid,i1,2)=fi(k,tid,i1,2)+f14v*r(k)
C              fi(k,tid,i4,2)=fi(k,tid,i4,2)-f14v*r(k)
C            enddo
C          endif
C
C          if (i2.lt.0) i2=-i2
...
C
        integer i,j,k,it
        double precision g0,r0,d0
        double precision rho,pi
        double precision rH,rCl,cH,cCl,qH,qCl,Rc,cC,qC
C     
        common /solvent/ gr,Emf,fLJ,vtot,e0,p0,q0,o0,rmax,eps,
     &       imin,styp,npts,ncpu,nstyp
C
...
C
        rC=dabs(d0)
        rH=dabs(1.1d0+d0)
        rCl=dsqrt(1.758d0**2+d0**2+2.d0*d0*1.758d0*dcos(1.87937134d0))
        if (abs(d0).lt.1.d-8) then
          cC=1.d0
        else
          cC=d0/rC
        endif
        cH=(1.1d0+d0)/rH
        if (abs(d0).lt.1.d-8) then
          cCl=dcos(1.87937134d0)
        else
          cCl=(d0**2+rCl**2-1.758d0**2)/2.d0/d0/rCl
        endif
        qH=0.2659d0
        qCl=3.d0*0.0396d0
        qC=-qH-qCl
C
        p0=dsqrt(332.d0)*(qC*rC*cC+qH*rH*cH+qCl*rCl*cCl)
        q0=dsqrt(332.d0)*(qH*rH**2*(1.5d0*cH**2-0.5d0)
     &                      +qCl*rCl**2*(1.5d0*cCl**2-0.5d0)
     &                      +qC*rC**2*(1.5d0*cC**2-0.5d0))
        o0=dsqrt(332.d0)*(qH*rH**3*(2.5d0*cH**2-1.5d0)*cH
     &                      +qCl*rCl**3*(2.5d0*cCl**2-1.5d0)*cCl
     &                      +qC*rC**3*(2.5d0*cC**2-1.5d0)*cC)
C
        e0=3.d0/4.d0/pi/rho/p0*(1.d0-1.d0/eps)
C
        open(20,FILE='LJ2.crd',STATUS='old')
        read(20,*) it,nstyp
        do i=1,it
          read(20,*) j
          styp(i)=j
        ...
        close(20)
C
        open(20,FILE='LJ_q1.0.SPA2.dat',STATUS='old')
        do i=1,nstyp
          do j=1,hmax
            read(20,*) r0,g0,it,k
            if(k.le.0) then
        ...
        close(20)
C
        open(20,FILE='LJ_q1.0.pol.dat',STATUS='old')
        do i=1,nstyp
          it=ttyp(i)
          do j=1,hmax
            read(20,*) r0,g0
        ...
        close(20)
C     
        open(20,FILE='LJ_q1.0.frc.dat',STATUS='old')
        do i=1,nstyp
          do j=1,hmax
            read(20,*) r0,g0
            fLJ(j,i)=g0
