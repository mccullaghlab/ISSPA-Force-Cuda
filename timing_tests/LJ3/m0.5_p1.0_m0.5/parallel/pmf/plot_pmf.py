...
for i,val in enumerate(values):
        ...
        DCDD_data = np.loadtxt("/home/ryan/Desktop/isspa/isspa2_v3_forces/timing_tests/LJ2/p1.0_m0.5/25/pmf.dat")
        ...
        for  i in range(len(DCDD_data)):
                z12 = DCDD_data[i,0]
                DCDD_data[i,1] =  -DCDD_data[i,1] + uCDD3z(q2,q3,z12,z13) + uLJ3z(z12,z13)                
        z12 = DCDD_data[index,0]
        DCDD_data[:,2] += -DCDD_data[index,1] + uCDD(q1,q2,z12) + uCDD3z(q2,q3,z12,z13) + uLJ(z12) + uLJ3z(z12,z13)                
        plt.plot(DCDD_data[:,0], DCDD_data[:,1], c = 'b', label = "$u_{pmf}^{12}$ + $u_{CDD}^{23}$ + $u_{LJ}^{23}$", linestyle='--')

        # Plot trimer PMF removing isspa interactions between 2-3
        data = np.loadtxt("/home/ryan/Desktop/isspa/isspa2_v3_forces/timing_tests/LJ3_23/m0.5_p1.0_m0.5/parallel/pmf/pmf.%s.dat" %(val))
        # Add in CDD + LJ from atom 3 to atom 1
        for i in range(len(data)):
                z12 = data[i,1]
        ...
        plt.xticks(np.arange(0, cutoff+1, 1.0))
        plt.xlim((0,cutoff))
        plt.ylim((-45.0, 5.0))
        plt.savefig('m0.5_p1.0_m0.5.%s.zz.pmf.pdf' %(val))
        plt.savefig('m0.5_p1.0_m0.5.%s.zz.pmf.png' %(val))
        plt.close()
