...
/*
	Notes from John.
		...
		This is a wrapper to the Hilbert curve code of J. K. Lawder, see info later in file.
		
		I adjusted a couple of lines to prevent compiler warning regarding missing braces around initialisers for
		some data; I've flagged these changes where they occur.
*/
...
Hilbert3D::Hilbert3D() {}
Hilbert3D::~Hilbert3D() {}
int Hilbert3D::CoordsToIndex( int x, int y, int z )
{
	Hcode v;
	Point pt;
...
void Hilbert3D::IndexToCoords( int index, int &x, int &y, int &z )
{
	Hcode v;
	Point pt;
	
	v.hcode[0] = index;
	v.hcode[1] = v.hcode[2] = 0;
...
Hcode H_encode(Point p)
{
	...
	Hcode	h = { {0,0,0} }; // JOHN - altered this! was "Hcode h = {0};"
...
Point H_decode (Hcode H)
{
	U_int	mask = (U_int)1 << WORDBITS - 1, element, temp1, temp2,
		A, W = 0, S, tS, T, tT, J, P = 0, xJ;
	Point	pt = { {0,0,0} }; // JOHN - altered this, was "Point	pt = {0};"
	int	i = NUMBITS * DIM - DIM, j;
