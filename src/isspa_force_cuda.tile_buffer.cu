...
__global__ void __launch_bounds__(32, 8) isspa_field_kernel(float4 *xyz, const float* __restrict__ rmax, int *isspaTypes, const float* __restrict__  gTable, const float* __restrict__  eTable, float4 *mcpos, float *buf_mcpos, float4 *buf_enow, float4 *buf_e0now) { 
	int atom;
        int bin;
        int jt;
	int mc;
	...
	float dist2, dist;
        ...
        if (mc < nMC*nAtoms) {                                
                ...
                buf_mcpos[mc*nRows+tRow] = mcpos_l.w;
                buf_enow[mc*nRows+tRow] = enow_l;
                buf_e0now[mc*nRows+tRow] = e0now_l;
                
        }
}

__global__ void __launch_bounds__(1024,1)  reduce_convert_kernel(float *buf_mcpos, float4 *buf_enow, float4 *buf_e0now, float4 *mcpos, float4 *enow, float4 *e0now) {
        ...
        buf_mcpos_l = 1.0f;
        buf_enow_l.x = buf_enow_l.y = buf_enow_l.z = 0.0f;
        buf_e0now_l.x = buf_e0now_l.y = buf_e0now_l.z = buf_e0now_l.w = 0.0f;
        mc = blockIdx.x*blockDim.x + threadIdx.x;

        if (mc < nMC*nAtoms) {
                unsigned int nRows = ceil(double(nAtoms)/double(32.0));
                for (unsigned int offset = 0; offset < nRows; offset ++) {
                        buf_mcpos_l *= buf_mcpos[mc*nRows+offset];
                        buf_enow_l.x += buf_enow[mc*nRows+offset].x;
                        buf_enow_l.y += buf_enow[mc*nRows+offset].y;
                        buf_enow_l.z += buf_enow[mc*nRows+offset].z;
                        buf_e0now_l.x += buf_e0now[mc*nRows+offset].x;
                        buf_e0now_l.y += buf_e0now[mc*nRows+offset].y;
                        buf_e0now_l.z += buf_e0now[mc*nRows+offset].z;
                        buf_e0now_l.w += buf_e0now[mc*nRows+offset].w;
                }
                mcpos[mc].w = buf_mcpos_l;
                enow[mc] = buf_enow_l;
                e0now[mc] = buf_e0now_l;
        }
...
float isspa_force_cuda(float4 *xyz_d, float4 *f_d, float4 *isspaf_d, isspa& isspas, int nAtoms_h) {
	...
	cudaMemset(isspaf_d,       0.0f,  nAtoms_h*sizeof(float4));
        ...
        //cudaMemset(isspas.buffer_mcpos_d, 1.0f, ceil(nAtoms_h/32.0)*nAtoms_h*isspas.nMC*sizeof(float));
        //cudaMemset(isspas.buffer_enow_d,  0.0f,  ceil(nAtoms_h/32.0)*nAtoms_h*isspas.nMC*sizeof(float4));
        //cudaMemset(isspas.buffer_e0now_d, 0.0f,  ceil(nAtoms_h/32.0)*nAtoms_h*isspas.nMC*sizeof(float4));
	...
	isspa_field_kernel<<<isspas.fieldGridSize, isspas.fieldBlockSize, 32*sizeof(float4)>>>(xyz_d, isspas.rmax_d, isspas.isspaTypes_d, isspas.isspaGTable_d, isspas.isspaETable_d, isspas.mcpos_d, isspas.buffer_mcpos_d, isspas.buffer_enow_d, isspas.buffer_e0now_d);

        // Reduce the fields and convert to polarizations
        reduce_convert_kernel<<<ceil(isspas.nMC*nAtoms_h/1024.0),1024>>>(isspas.buffer_mcpos_d, isspas.buffer_enow_d, isspas.buffer_e0now_d, isspas.mcpos_d, isspas.enow_d, isspas.e0now_d);
