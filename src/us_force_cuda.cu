...
__global__ void us_force_kernel(float4 *xyz, float4 *f, int totalBiasAtoms, int2 *atomList, float *mass, float4 *groupComPos, float *kumb, float x0, float lbox) {
	...
	if (index < totalBiasAtoms) {
		...
		f[atom.x] += kumb[atom.y]*(comSepVec.w-x0)*comSepVec*threadMass; 
	}

}
/* C wrappers for kernels */

float us_force_cuda(float4 *xyz_d, float4 *f_d, us& bias, float lbox) {
	...
	us_force_kernel<<<bias.usGridSize, bias.usBlockSize>>>(xyz_d, f_d, bias.totalBiasAtoms, bias.atomList_d, bias.mass_d, bias.groupComPos_d, bias.kumb, bias.x0, lbox);
