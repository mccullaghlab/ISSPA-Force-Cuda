...
__global__ void isspa_force_kernel(float4 *xyz, float *vtot, float *rmax, int *isspaTypes, float *forceTable, float4 *f, curandState *state,  float4 *enow, float4 *e0now, float4 *mcpos, int4 CalcsPerThread, float4 *isspaf) {
	unsigned int tid = threadIdx.x;
        int bin;
        int jt;
	int i;
...
//float isspa_force_cuda(float4 *xyz_d, float4 *f_d, isspa& isspas, int nAtoms_h) {
	...
	isspa_force_kernel<<<isspas.fGridSize, isspas.fBlockSize>>>(xyz_d,isspas.vtot_d,isspas.rmax_d,isspas.isspaTypes_d,isspas.isspaForceTable_d,f_d,isspas.randStates_d,isspas.enow_d,isspas.e0now_d,isspas.mcpos_d,isspas.fCalcsPerThread,isspaf_d);
...
void isspa_grid_block(int nAtoms_h, int nPairs_h, float lbox_h, isspa& isspas) {
	...
	isspas.fGridSize = nAtoms_h;
	if (isspas.nMC*nAtoms_h < maxThreadsPerBlock) {
	  temp = int(ceil(isspas.nMC*nAtoms_h /(float) 32));
	  isspas.fCalcsPerThread.y = temp*32;
	  isspas.fBlockSize = temp*32;		
	} else {
	  isspas.fBlockSize = maxThreadsPerBlock;
	  isspas.fCalcsPerThread.y = maxThreadsPerBlock;	  	
	}
	isspas.fCalcsPerThread.x = int(ceil(nAtoms_h*isspas.nMC/ (float) isspas.fBlockSize));	  	
