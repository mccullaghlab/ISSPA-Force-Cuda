...
__global__ void isspa_field_kernel(float4 *xyz, const float* __restrict__ rmax, int *isspaTypes, const float* __restrict__  gTable, const float* __restrict__  eTable, float4 *enow, float4 *e0now, float4 *mcpos, int nThreads) { 
	...
	int jt;
        ...
        atom = int(blockIdx.x/nThreads);
        MCind = int(threadIdx.x/32.0f + (blockIdx.x - atom*nThreads)*32);
...
float isspa_force_cuda(float4 *xyz_d, float4 *f_d, float4 *isspaf_d, isspa& isspas, int nAtoms_h) {
	...
	isspa_field_kernel<<<isspas.fieldGridSize, isspas.fieldBlockSize, nAtoms_h*sizeof(float4)>>>(xyz_d, isspas.rmax_d, isspas.isspaTypes_d, isspas.isspaGTable_d, isspas.isspaETable_d, isspas.enow_d, isspas.e0now_d, isspas.mcpos_d, isspas.fieldThreads);
...
void isspa_grid_block(int nAtoms_h, int nPairs_h, float lbox_h, isspa& isspas) {
	...
	// determine gridSize and blockSize for field kernel	
        isspas.fieldThreads = int(ceil(isspas.nMC / 32.0));		
	isspas.fieldGridSize = int(ceil(isspas.nMC / (float) 32.0))*nAtoms_h;
