...
__global__ void neighborlist_kernel(float4 *xyz, int4 *neighborList, int *neighborCount, float rNN2, int nAtoms, float lbox, int *nExcludedAtoms, int *excludedAtomsList, int excludedAtomsListLength, int *nbparm, int *ityp, int nTypes, int nPairs) {
...
float neighborlist_cuda(atom& atoms, float rNN2, float lbox)
{
	...
	// zero neighborCount
	cudaMemset(atoms.neighborCount_d,0, sizeof(int));
	// run nonbond cuda kernel
	neighborlist_kernel<<<gridSize, blockSize, atoms.excludedAtomsListLength*sizeof(int)>>>(atoms.pos_d, atoms.neighborList_d, atoms.neighborCount_d, rNN2, atoms.nAtoms, lbox, atoms.nExcludedAtoms_d, atoms.excludedAtomsList_d, atoms.excludedAtomsListLength, atoms.nonBondedParmIndex_d, atoms.ityp_d, atoms.nTypes, atoms.nPairs);
	// copy neighbor count to host and print
	cudaMemcpy(atoms.neighborCount_h, atoms.neighborCount_d,sizeof(int),cudaMemcpyDeviceToHost);
